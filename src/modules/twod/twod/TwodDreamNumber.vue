<template>
    <section class="flex flex-col items-center p-3 !bg-secondary">
        ...
        <div class="grid grid-cols-4 gap-2">
            <div v-for="item in items" :key="item.id" class="bg-primary text-light rounded-lg space-y-3 shadow-md p-2">
                ...
                <div class="flex space-x-2">
                    <span v-for="child in item.children" :key="child.number" class="bg-secondary cursor-pointer text-white p-2 rounded-md text-sm font-medium">
                        {{ child.number }}
                    </span>
                </div>
            </div>
        ...
        <div class="flex justify-center space-x-4 mt-10 mb-24">
            <button @click="changePage(previousPageUrl)" :disabled="!previousPageUrl" class="px-4 py-2 bg-primary text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed">
                {{ $t('prev') }}
            </button>
            <button @click="changePage(nextPageUrl)" :disabled="!nextPageUrl" class="px-4 py-2 bg-primary text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed">
                {{ $t('next') }}
            </button>
        </div>
    </section>
